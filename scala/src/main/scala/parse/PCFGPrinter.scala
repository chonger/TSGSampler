...
object PCFGPrinter {
  ...
  def printSplits(id : ParseTypes.Symbol) = {
    var x = pcfg.splitTrees(id)
    printSplitTree(x)
  }
  
  def printSplitTree(x : List[List[Tuple2[ParseTypes.Split,Int]]]) : String = {
    var ret = for{i <- 0 to x.length - 1} yield {
      ("" /: x(i))((a,b) => {a + " " + b._1.toInt + "/" + b._2})
    }
    ("" /: ret)((a,b) => a + "\n" + b)
  }
  def printSplitsAtDepth(id : ParseTypes.Symbol, depth : Int) : String = {
    var x = splitTrees(id)(depth)
