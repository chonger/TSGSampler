...
void TreeChunker::resample(size_t iterations) {
    ...
    for(size_t i=0;i<iterations;++i) {
        ...
        if(nThreads > 1) {
            ...
            //reduce dps
            dp->fromTreeData(); //sets the cache and lhs counts for the master DP

            //wastes time, but oh so much easier to code...to be efficient, fix the commented stuff
            for(size_t j=0;j<nThreads;++j) {
                threads[j]->dp->fromTreeData(); 
            }
            
            /**
            //clear the cache of all of all threads
            for(size_t j=0;j<nThreads;++j) {
                for(size_t i=0;i<tData->nLHS;++i) {
                    threads[j]->dp->lhsCounts[i] = 0;
                }
